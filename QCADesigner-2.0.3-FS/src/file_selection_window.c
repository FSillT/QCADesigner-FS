...
gchar *get_path_name_from_user (GtkWindow *parent, char *pszWinTitle, char *pszFName, gboolean bOverwritePrompt)
  {
  gchar *pszRet = NULL ;
  gulong handlerID = 0 ;
  ...
  if (NULL != (pszRet = get_default_file_name (GTK_FILE_SELECTION (file_selection), pszFName)))
    {
    gtk_file_selection_set_filename (GTK_FILE_SELECTION (file_selection[2]), pszRet) ;
    g_free (pszRet) ;
    pszRet = NULL ;
    }
  ...
  if ((GTK_RESPONSE_OK == gtk_dialog_run (GTK_DIALOG (file_selection))))
    pszRet = g_strdup_printf ("%s", gtk_file_selection_get_selections (GTK_FILE_SELECTION (file_selection))) ;
  gtk_widget_hide (file_selection) ;
