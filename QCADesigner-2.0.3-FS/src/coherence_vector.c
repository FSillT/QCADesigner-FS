...
static void run_coherence_iteration (int sample_number, int number_of_cell_layers, int *number_of_cells_in_layer, QCADCell ***sorted_cells, int total_number_of_inputs, unsigned long int number_samples, const coherence_OP *options, simulation_data *sim_data, int SIMULATION_TYPE, int SIMULATION_MODE, VectorTable *pvt);

static inline double calculate_clock_value (unsigned int clock_num, unsigned long int sample, unsigned long int number_samples, int total_number_of_inputs, const coherence_OP *options, int SIMULATION_TYPE, int SIMULATION_MODE, VectorTable *pvt);
...
//-------------------------------------------------------------------//
simulation_data *run_coherence_simulation (int SIMULATION_TYPE, int SIMULATION_MODE, DESIGN *design, coherence_OP *options, VectorTable *pvt)
  {
  ...
  if (SIMULATION_MODE==CLOCK_PHASE_SHIFTS_SIMULATION)
  {
  if(SIMULATION_TYPE == VECTOR_TABLE)
  {
  number_recorded_samples=ceil(number_recorded_samples*(1.00-(((double)pvt->vectors->icUsed*options->time_step*fault_simulation_options.frequency_shift)/options->duration)));
  number_samples = (unsigned long int)(ceil (options->duration/options->time_step-pvt->vectors->icUsed*fault_simulation_options.frequency_shift));
  }
  else
  {
  number_recorded_samples=ceil(number_recorded_samples*(1.00-((4.00*options->time_step*fault_simulation_options.frequency_shift)/options->duration)));
  number_samples = (unsigned long int)(ceil (options->duration/options->time_step-4*fault_simulation_options.frequency_shift));
  }
  }
  else
  ...
  for (i = 0; i < design->bus_layout->inputs->icUsed; i++)
    ...
    sim_data->trace[i].clockzone = QCAD_CELL(exp_array_index_1d (design->bus_layout->inputs, BUS_LAYOUT_CELL, i).cell)->cell_options.clock;
    }
  ...
  for (i = 0; i < design->bus_layout->outputs->icUsed; i++)
    ...
    //sim_data->trace[i + total_number_of_inputs].clockzone = QCAD_CELL(design->bus_layout->outputs[i].data)->cell_options.clock;
    sim_data->trace[i + total_number_of_inputs].clockzone = QCAD_CELL(exp_array_index_1d (design->bus_layout->outputs, BUS_LAYOUT_CELL, i).cell)->cell_options.clock;
    }
  ...
  for (i = 0; i < 4; i++)
    ...
    sim_data->clock_data[i].clockzone=NULL;
    ...
    for (j = 0; j<sim_data->number_samples; j++)
      ...
      sim_data->clock_data[i].data[j] = calculate_clock_value(i, j * record_interval, number_samples, total_number_of_inputs, options, SIMULATION_TYPE, SIMULATION_MODE, pvt);
    }
  ...
  for (j = 0; j < number_samples; j++)
    ...
    if (EXHAUSTIVE_VERIFICATION == SIMULATION_TYPE)
      for (idxMasterBitOrder = 0, design_bus_layout_iter_first (design->bus_layout, &bli, QCAD_CELL_INPUT, &i) ; i > -1 ; design_bus_layout_iter_next (&bli, &i), idxMasterBitOrder++)
        {
        qcad_cell_set_polarization (exp_array_index_1d (design->bus_layout->inputs, BUS_LAYOUT_CELL, i).cell,
          dPolarization = (-sin (((double) (1 << idxMasterBitOrder)+idxMasterBitOrder*fault_simulation_options.frequency_shift/(double)ceil(number_samples+4.00*fault_simulation_options.frequency_shift)) * (double) j * optimization_options.four_pi_over_number_samples)) > 0 ? 1 : -1) ;
...
//    if (VECTOR_TABLE == SIMULATION_TYPE)
    ...
    run_coherence_iteration (j, number_of_cell_layers, number_of_cells_in_layer, sorted_cells, total_number_of_inputs, number_samples, options, sim_data, SIMULATION_TYPE, SIMULATION_MODE, pvt);
...
static void run_coherence_iteration (int sample_number, int number_of_cell_layers, int *number_of_cells_in_layer, QCADCell ***sorted_cells, int total_number_of_inputs, unsigned long int number_samples, const coherence_OP *options, simulation_data *sim_data, int SIMULATION_TYPE, int SIMULATION_MODE, VectorTable *pvt)
  ...
  for (i = 0 ; i < number_of_cell_layers ; i++)
    for (j = 0 ; j < number_of_cells_in_layer[i] ; j++)
      ...
      clock_value = calculate_clock_value(sorted_cells[i][j]->cell_options.clock, sample_number, number_samples, total_number_of_inputs, options, SIMULATION_TYPE, SIMULATION_MODE, pvt);
      PEk = 0;
...
static inline double calculate_clock_value (unsigned int clock_num, unsigned long int sample, unsigned long int number_samples, int total_number_of_inputs, const coherence_OP *options, int SIMULATION_TYPE, int SIMULATION_MODE, VectorTable *pvt)
  {
  double clock = 0;
  if ((SIMULATION_MODE == REGULAR) || (SIMULATION_MODE == DEFECTS_SIMULATION))
  {
  ...
  else
  {
   if (SIMULATION_MODE == CLOCK_PHASE_SHIFTS_SIMULATION)
   {
    ...
    if (SIMULATION_TYPE == EXHAUSTIVE_VERIFICATION)
      {
      clock = optimization_options.clock_prefactor *
	cos (((double)(1 << total_number_of_inputs)+fault_simulation_options.frequency_shift*options->time_step) *
	      (double) sample * optimization_options.four_pi_over_number_samples - PI * (double)clock_num * 0.5 - phase_shift[clock_num]) + optimization_options.clock_shift + options->clock_shift;
    ...
    if (SIMULATION_TYPE == VECTOR_TABLE)
      {
      clock = optimization_options.clock_prefactor *
        cos (((double)pvt->vectors->icUsed+fault_simulation_options.frequency_shift*options->time_step) *
              (double) sample * optimization_options.two_pi_over_number_samples - PI * (double)clock_num * 0.5 - phase_shift[clock_num]) + optimization_options.clock_shift + options->clock_shift;
