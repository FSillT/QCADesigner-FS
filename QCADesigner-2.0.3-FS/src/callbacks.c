...
//Dayane Alfenas Reis
extern fault_simulation_OP fault_simulation_options;
extern analysis_fault_simulation_OP analysis_summary;
extern analysis_result_fault_simulation_OP analysis_result;
extern double phase_shift[4]={0,0,0,0};
//End of Dayane Alfenas Reis - 10/13/14 03:55PM
...
//!General CAD options
  FALSE,                     // SHOW_USE
//!General CAD options (Size of USE clock scheme)
  5,                     // USE_size
//End of Dayane
...
gboolean scroll_event(GtkWidget *widget, GdkEventScroll *event, gpointer user_data)
  ...
  //dayane
  //if (project_options.SHOW_USE==TRUE)
  //show_hide_USE(main_window.drawing_area,NULL);
  //end of dayane
...
//Dayane Alfenas Reis - 10/29/14 -9:31PM

void on_USE_clocking_scheme_setup_menu_item_activate (GtkMenuItem * menuitem, gpointer user_data)
{
  DBG_CB_HERE (fprintf (stderr, "Entering on_USE_clocking_scheme_setup_menu_item_activate\n")) ;
  project_options.USE_size=get_USE_size_from_user (GTK_WINDOW (main_window.main_window), project_options.USE_size) ;
}  //on_USE_clocking_scheme_setup_menu_item_activate


void on_show_USE_clocking_scheme_menu_item_activate (GtkMenuItem * menuitem, gpointer user_data)
  {
  DBG_CB_HERE (fprintf (stderr, "Entering on_show_USE_clocking_scheme_menu_item_activate\n")) ;
  project_options.SHOW_USE = ((GtkCheckMenuItem *) menuitem)->active;

  if (project_options.SHOW_USE==TRUE)
  show_USE(main_window.drawing_area,NULL);
  else
  redraw_async (NULL);
...
//dayane
void show_USE (GtkWidget *widget, gpointer data)
{
GdkGC *gc;
GdkColor *clr = NULL;
double dcx, dcy, x1, y1, x, y;
int x1_real, y1_real, x2_real, y2_real, n,k, iClock;
dcx=QCAD_CELL (project_options.design)->cell_options.cxCell;
dcy=QCAD_CELL (project_options.design)->cell_options.cyCell;

k=20; //espacamento do grid

//Variavel recebendo o valor do num de celulas do grid
n=project_options.USE_size;

	for (x=k/2;x<=6000;x=x+k*n)
	{
		for (y=k/2;y<=3000;y=y+k*n)
		{
			x1_real=world_to_real_x (x);
			y1_real=world_to_real_y (y);
			x2_real=world_to_real_x (x+k*n);
			y2_real=world_to_real_y (y+k*n);

			//Determina o quadrante atraves do resto da divisao
			if (x-dcx/2>4*k*n)
				x1=fmod(x-dcx/2,(4*k*n));
			else
				x1=(x-dcx/2);

			if (y-dcy/2>4*k*n)
				y1=fmod(y-dcy/2,(4*k*n));
			else
				y1=(y-dcy/2);

			//Atribui automaticamente a zona de clock segundo a posicao
			if (((x1<=k*n) & (y1<=k*n))
					+ ((x1>3*k*n) & (x1<=4*k*n) & (y1>k*n) & (y1<=2*k*n))
					+ ((x1>2*k*n) & (x1<=3*k*n) & (y1>2*k*n) & (y1<=3*k*n))
					+ ((x1>k*n) & (x1<=2*k*n) & (y1>3*k*n) & (y1<=4*k*n)))
			{
				iClock=0;

				gc = gdk_gc_new (widget->window) ;
				gdk_gc_set_foreground (gc, clr = clr_idx_to_clr_struct (GREEN)) ;
				gdk_gc_set_background (gc, clr) ;
				gdk_gc_set_line_attributes (gc, 1, GDK_LINE_ON_OFF_DASH, GDK_CAP_BUTT, GDK_JOIN_MITER);
				gdk_draw_line (widget->window, gc, x1_real, y1_real, x2_real, y1_real) ;
				gdk_draw_line (widget->window, gc, x1_real, y1_real, x1_real, y2_real) ;

				g_object_unref (gc) ;
				//break;
			}

			if (((x1>k*n) & (x1<=2*k*n) & (y1<=k*n))
					+ ((x1>2*k*n) & (x1<=3*k*n) & (y1>k*n) & (y1<=2*k*n))
					+ ((x1>3*k*n) & (x1<=4*k*n) & (y1>2*k*n) & (y1<=3*k*n))
					+ ((x1<=k*n) & (y1>3*k*n) & (y1<=4*k*n)) )
			{
				iClock=1;

				gc = gdk_gc_new (widget->window) ;
				gdk_gc_set_foreground (gc, clr = clr_idx_to_clr_struct (RED)) ;
				gdk_gc_set_background (gc, clr) ;
				gdk_gc_set_line_attributes (gc, 1, GDK_LINE_ON_OFF_DASH, GDK_CAP_BUTT, GDK_JOIN_MITER);
				gdk_draw_line (widget->window, gc, x1_real, y1_real, x2_real, y1_real) ;
				gdk_draw_line (widget->window, gc, x1_real, y1_real, x1_real, y2_real) ;
			...
			if (((x1>2*k*n) & (x1<=3*k*n) & (y1<=k*n))
					+ ((x1>k*n) & (x1<=2*k*n) & (y1>k*n) & (y1<=2*k*n))
					+ ((x1<=k*n) & (y1>2*k*n) & (y1<=3*k*n))
					+ ((x1>3*k*n) & (x1<=4*k*n) & (y1>3*k*n) & (y1<=4*k*n)) )
			{
				iClock=2;
			...
			if (((x1>3*k*n) & (x1<=4*k*n) & (y1<=k*n))
					+ ((x1<=k*n) & (y1>k*n) & (y1<=2*k*n))
					+ ((x1>k*n) & (x1<=2*k*n) & (y1>2*k*n) & (y1<=3*k*n))
					+ ((x1>2*k*n) & (x1<=3*k*n) & (y1>3*k*n) & (y1<=4*k*n)) )
			{
				iClock=3;

				gc = gdk_gc_new (widget->window) ;
				gdk_gc_set_foreground (gc, clr = clr_idx_to_clr_struct (WHITE)) ;
				gdk_gc_set_background (gc, clr) ;
				gdk_gc_set_line_attributes (gc, 1, GDK_LINE_ON_OFF_DASH, GDK_CAP_BUTT, GDK_JOIN_MITER);
				gdk_draw_line (widget->window, gc, x1_real, y1_real, x2_real, y1_real) ;
				gdk_draw_line (widget->window, gc, x1_real, y1_real, x1_real, y2_real) ;
...
//Definicao das variaveis
  ...
  if (project_options.SHOW_USE == TRUE)
  {
	  ...
	  //se está fora do substrato, retorna iClock=4
	  if (x<k/2 || x>6000 || y<k/2 || y>3000)
	  return iClock=4;
...
//End of Dayane Alfenas Reis - 10/29/14 -9:31PM
...
////////////// Dayane Alfenas Reis - 06/10/14 - 01:26pm

void on_fault_simulation_start_menu_item_activate(GtkMenuItem * menuitem, gpointer user_data)
{
	  gboolean bSimOK = TRUE ;
	  FILE *fp1 = NULL ;
	  char *pszFName = NULL, *resultsFName = NULL, *designFName = NULL, *config_filename = NULL ;
	  char *pszTempFName = NULL ;
	  char *ind = NULL;
	  DESIGN des_referencia;
	  SIMULATION_OUTPUT sim_output_ref;
	  int n,ns, ncell, totalcell, ph_a, ph_b, ph0, ph1, ph2, ph3, i, j, n_shifts, signal, n_min, n_max;
	  int volatile nsym;
	  int countsym, last_sym, count_pw=0;
	  float nrand, nselect, step, range_step;
	  double volatile range_min, range_max;
	  GList *id_cell = NULL, *id_cell2 = NULL;
	  //Cria o vetor percent_working para finalizar iteracoes automaticamente
	  gfloat *percent_working = NULL;
	  //Cria variaveis para media
	  float volatile razao=1, razao_ref=1;

	    DBG_CB_HERE (fprintf (stderr, "Entering on_fault_simulation_start_menu_item_activate\n")) ;
	    ...
	    if (!bSimOK)
  	    ...
  	    if (NULL == (pszTempFName = get_file_name_from_user (GTK_WINDOW (main_window.main_window), _("Output file Path"), pszFName, TRUE)))
  	    return ;
  	    ...
  	    //Set simulation mode
  	    if (fault_simulation_options.kind_of_fault==0)
  	    project_options.SIMULATION_MODE=DEFECTS_SIMULATION;

  	    if (fault_simulation_options.kind_of_fault==1)
  	    project_options.SIMULATION_MODE=CLOCK_PHASE_SHIFTS_SIMULATION;

	     //First of all, run the simulation for the original design
	      ...
	      {
	    	  sim_output_ref.sim_data = project_options.sim_data;
	    	  sim_output_ref.bus_layout = project_options.design->bus_layout;
	    	  sim_output_ref.bFakeIOLists=FALSE ;
	      }

	      //Create an output file for the design / simulation
	      create_simulation_output_file(g_strdup_printf ("%s%s", _(pszFName), "_original"), &sim_output_ref) ;
	      create_file (g_strdup_printf ("%s%s", _(pszFName), "_original_design"), project_options.design) ;

	      //Limpa a contagem clear simulations
	      analysis_summary.clear_simulations=0;

	      countsym=fault_simulation_options.inicial_ID;
		 //Number of fault simulations
		  ns= fault_simulation_options.number_of_simulations;
		 //aloca memoria para o vetor que registra a evolucao das simulacoes
		  percent_working = malloc(ns * sizeof(float));


	      if (project_options.SIMULATION_MODE==DEFECTS_SIMULATION)
	{

	      totalcell=0;

	      for (id_cell = project_options.design->lstLayers; id_cell!=NULL; id_cell=id_cell->next)
	      {
	      	for (id_cell2=QCAD_LAYER(id_cell->data)->lstObjs; id_cell2!=NULL; id_cell2=id_cell2->next)
	      	{
	      	    if (QCAD_IS_CELL(id_cell2->data)==TRUE)
	      	    totalcell=totalcell+1; 	    //count how many cells are in the design
	      	}
	      }

	      if (fault_simulation_options.p_engine==2)//step
		{
	    	  //realloc(percent_working, (ns * totalcell));

	 		 //Number of fault simulations - recalcular
	 		  //ns= n*totalcell;

			     if (countsym==fault_simulation_options.inicial_ID)
			     nsym=(int)((fault_simulation_options.inicial_ID-1)/totalcell)+1;
			     else
			     nsym=1;

				for (n=nsym; n<=fault_simulation_options.number_of_simulations; n++)
				{

					for (ncell = fmod(fault_simulation_options.inicial_ID-1,totalcell)+1 ; ncell<= totalcell ; ncell++)
					{

		  	    	start_fault_simulation_step(project_options, pszFName, pvt, n, ncell, totalcell, countsym);
		  	    	///////////
		  	    	resultsFName= g_strdup_printf ("%s%s", _(pszFName), "_results");
		  	    	config_filename= g_strdup_printf ("%s%s", _(pszFName), "_config");
		  	    	designFName= g_strdup_printf ("%s%s", _(pszFName), "_original_design");

		  	    	percent_working[countsym-1]=synthetize_and_create_hm(resultsFName, designFName, config_filename, n);

		  	    	//contador para parar se estavel - REVISAR
		  	    	//if ((countsym>1) && (countsym<=ns))
		  	    	//{
			  	    //	razao=percent_working[countsym-1]/percent_working[countsym-2];

			  	    //	if (countsym==1)
			  	    //	razao_ref=razao;

			  	    //	if (fabs(razao-razao_ref)<=(float)0.005)
			  	    //	count_pw++;
			  	    //	else
			  	    //	count_pw=0;
			  	    //	razao_ref=razao;
		  	    	...
		  	    	if (countsym<(fault_simulation_options.number_of_simulations*totalcell))
		  	    	countsym++;

		  	    	//verifica se estavel para sair do ciclo
		  	    	//if (count_pw>=fault_simulation_options.number_of_simulations*0.02)
		  	    	//{
		  	    	//count_pw=0;
		  	    	//break;
		  	    	//}
	      ...
	      if (fault_simulation_options.p_engine==1)//uniform
	      {

		      for (n = countsym ; n<= ns ; n++)
		      {
			  start_fault_simulation_uniform(project_options, pszFName, pvt, n, totalcell, countsym);
				///////////
		  	    	resultsFName= g_strdup_printf ("%s%s", _(pszFName), "_results");
		  	    	config_filename= g_strdup_printf ("%s%s", _(pszFName), "_config");
		  	    	designFName= g_strdup_printf ("%s%s", _(pszFName), "_original_design");
		  	    	...
		  	    	//contador para parar se estavel
		  	    	if ((countsym>1) && (countsym<=ns))
		  	    	{
			  	    	razao=percent_working[countsym-1]/percent_working[countsym-2];
	      ...
	      if (fault_simulation_options.p_engine==0)//assignable (defects)
		  {

		      for (n = countsym ; n<= ns ; n++)
		      {
			  start_fault_simulation_ud(project_options, pszFName, pvt, n, totalcell, countsym);
				///////////
		  	    	resultsFName= g_strdup_printf ("%s%s", _(pszFName), "_results");
		  	    	config_filename= g_strdup_printf ("%s%s", _(pszFName), "_config");
		  	    	designFName= g_strdup_printf ("%s%s", _(pszFName), "_original_design");
	      ...
	      if (project_options.SIMULATION_MODE==CLOCK_PHASE_SHIFTS_SIMULATION)
	     {

		if (fault_simulation_options.p_engine_shifts==0)//sequential (phase shifts)
		{
		countsym=fault_simulation_options.inicial_ID;

		for (n=fault_simulation_options.inicial_ID; n<=ns; n++)
		{
		    //all possible combinations must be tested
		      for(ph0=0;ph0<=1;ph0++)
			  for(ph1=0;ph1<=1;ph1++)
			      for(ph2=0;ph2<=1;ph2++)
				  for(ph3=0;ph3<=1;ph3++)
				      {
				      if(ph0+ph1+ph2+ph3!=0)
				      {

					nrand=(float)rand()/RAND_MAX;
					phase_shift[0]=PI*fault_simulation_options.initial_shift+
						(ph0*PI*(fault_simulation_options.final_shift-fault_simulation_options.initial_shift)*(double)nrand);

					nrand=(float)rand()/RAND_MAX;
					phase_shift[1]=PI*fault_simulation_options.initial_shift+
						(ph1*PI*(fault_simulation_options.final_shift-fault_simulation_options.initial_shift)*(double)nrand);

					nrand=(float)rand()/RAND_MAX;
					phase_shift[2]=PI*fault_simulation_options.initial_shift+
						(ph2*PI*(fault_simulation_options.final_shift-fault_simulation_options.initial_shift)*(double)nrand);

					nrand=(float)rand()/RAND_MAX;
					phase_shift[3]=PI*fault_simulation_options.initial_shift+
						(ph3*PI*(fault_simulation_options.final_shift-fault_simulation_options.initial_shift)*(double)nrand);

				      start_clock_phase_shifts_simulation(project_options, pszFName, pvt, n, countsym);
				      for (i=1;i<4;i++)
				      phase_shift[i]=0;
				      countsym++;
				      }
		 ...
		 if (fault_simulation_options.p_engine_shifts==1)//uniform random (phase shifts)
		  {
		  countsym=fault_simulation_options.inicial_ID;
	          ...
	          for (n = fault_simulation_options.inicial_ID ; n<= ns ; n++)
		  {
	              ...
	              //nselect=(float)rand()/RAND_MAX;
	              n_min=(int)(((float)rand()/RAND_MAX)/0.0833);
	              if (n_min>=12)
	              n_min=n_min-1;

	              n_max=n_min+1;

	              //gera deslocamentos aleatorios dentro de um intervalo definido da faixa
	              range_max= (float)(fault_simulation_options.initial_shift+
	        	      fabs(n_max*(fault_simulation_options.final_shift-fault_simulation_options.initial_shift))/12);
	              range_min= (float)(fault_simulation_options.initial_shift+
	        	      fabs(n_min*(fault_simulation_options.final_shift-fault_simulation_options.initial_shift))/12);

	              //gera deslocamentos aleatorios dentro de qualquer intervalo da faixa
	              //range_max= (float)(fault_simulation_options.initial_shift+
	              //fabs(fault_simulation_options.final_shift-fault_simulation_options.initial_shift));
	              //range_min= (float)fault_simulation_options.initial_shift;

	              for (i=0; i<4; i++)
			  {
		              signal=(rand()%2);
		              if (signal==0) //negativo
		              signal=-1;

			      nrand=(float)rand()/RAND_MAX;
			      phase_shift[i]=(double)signal*PI*((range_max-range_min)*(double)nrand+range_min);

			  }

		      start_clock_phase_shifts_simulation(project_options, pszFName, pvt, ns, countsym);

		      resultsFName= g_strdup_printf ("%s%s", _(pszFName), "_results");
		      config_filename= g_strdup_printf ("%s%s", _(pszFName), "_config");

		      percent_working[countsym-1]=synthetize_phase_shifts_sim_results(resultsFName, config_filename, n);
		  ...
		  if (fault_simulation_options.p_engine_shifts==2)//fixed (random shifts)
		  {
		  countsym=fault_simulation_options.inicial_ID;
		  ...
		  for (n = fault_simulation_options.inicial_ID ; n<= ns ; n++)
		      ...
		      nrand=(float)rand()/RAND_MAX;
		      phase_shift[fault_simulation_options.fixed_clock_phase]=PI*fault_simulation_options.initial_shift+
			      (PI*(fault_simulation_options.final_shift-fault_simulation_options.initial_shift)*(double)nrand);
	      ...
	      project_options.SIMULATION_MODE=REGULAR;

              //Escreve no arquivo de configuracao
		    config_filename=g_strdup_printf ("%s%s", _(pszFName), "_config");
		    if (NULL == (fp1 = fopen (config_filename, "a")))
		      return ;
		    else
		    {
	            fprintf (fp1, "[OBJECTIVE FUNCTION VALUES]\n") ;
	            for (i=0;i<countsym;i++)
	            fprintf (fp1, g_strdup_printf ("%f%s", percent_working[i], " ")) ;
	            fprintf (fp1, "\n[#OBJECTIVE FUNCTION VALUES]\n") ;
	            file_close_and_unbuffer (fp1) ;
	            //g_free(percent_working);

		    }

}//on_fault_simulation_start_menu_item_activate

void on_fault_simulation_results_menu_item_activate(GtkMenuItem * menuitem, gpointer user_data)
{
    //teste da janela -- OK!
    get_sim_fault_read_from_user (GTK_WINDOW (main_window.main_window), &analysis_result) ;

}  //on_fault_simulation_results_menu_item_activate

void on_fault_simulation_setup_menu_item_activate(GtkMenuItem * menuitem, gpointer user_data)
{
  DBG_CB_HERE (fprintf (stderr, "Entering on_fault_simulation_setup_menu_item_activate\n")) ;
  get_sim_fault_from_user (GTK_WINDOW (main_window.main_window), &fault_simulation_options) ;
}  //on_fault_simulation_setup_menu_item_activate

///////////// end of Dayane Alfenas Reis - 06/10/14 - 01:26pm
