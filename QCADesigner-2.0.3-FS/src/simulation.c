...
simulation_data *run_simulation (int sim_engine, int sim_type, int sim_mode, DESIGN *design, VectorTable *pvt)
  {
  switch (sim_engine)
    {
    case BISTABLE:
      return run_bistable_simulation(sim_type, sim_mode, design, &bistable_options, pvt);

    case COHERENCE_VECTOR:
      return run_coherence_simulation(sim_type, sim_mode, design, &coherence_options, pvt);
    }
  return NULL ;
  }//run_simulation
